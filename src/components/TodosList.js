...
export default function TodosList(props) {
    ...
    const deleteTodo = (event)=>{
        ...
        unique = unique.filter((e,index)=>{return index.toString() !== target.className});
    ...
    return (
        <>
            {todosList.map((e,index)=>{
                return (<li key={index.toString()} className="list">
                    {e} 
                    <textarea className={"editTask",index} style={{display:"none"}} onChange={()=>changeEditTodoState(event)} value={editTodoInput} key={`inp${e}`} type="text" placeholder="Edit here..." /><br/>
                    <button className={"saveTask",index} onClick={saveEditTodo} style={{display:"none"}} disabled={!editTodoInput} key={`save${e}`} >save</button><br/>
                    <button className={"edit",index}  key={`edit${index}`} onClick={()=>editTodo(event)}>Edit</button><br/>
                    <button key={`delete${index}`} className={"delete",index} onClick={()=>deleteTodo(event)}>Delete</button><br/>
                    </li>)})}
        </>
        );
    }